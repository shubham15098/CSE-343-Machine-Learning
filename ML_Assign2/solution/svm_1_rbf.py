...
def pred_method(mesh,X,Y,check_assign):
	...
	final1 = np.zeros((len(mesh),len(label)))
	...
	for k in label:
		...
		for i1 in range(len(mesh)):
			sum11 = 0
			for i2 in range(len(s)):
				sum11 = sum11 + (np.exp(-1*np.linalg.norm(mesh[i1]-s[i2])**2))*z[0][i2]
	...
	for i1 in range(len(final1)):
		max11 = -1000000
		index11 = -100000
		for i2 in range(len(final1[0])):
			if(final1[i1][i2] >= max11):
				max11 = final1[i1][i2]
				index11 = i2
...
for itr in range(5):
	...
	for k in label:
		...
		for i1 in range(len(test_x)):
			sum11 = 0
			for i2 in range(len(s)):
				sum11 = sum11 + (np.exp(-1*np.linalg.norm(test_x[i1]-s[i2])**2))*z[0][i2]
